...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		$st=$row["enlem2"].",". $row["boylam2"];
		$str=$str.",".$row["enlem2"].",". $row["boylam2"];
		
		
		$domain = strstr($row["enlem2"], '-');
		if($domain==$row["enlem2"]){
	$en1=floatval(substr($domain,1));
	 $en1=$en1*-1;
		}
		else 	$en1=floatval($row["enlem2"]);

	$domain = strstr($row["boylam2"], '-');
		if($domain==$row["boylam2"]){
	$boy1=floatval(substr($domain,1));
	 $boy1=$boy1*-1;
		}
		else $boy1=floatval($row["boylam2"]);
...
if ($result->num_rows >0) {
	...
	$sokete_gonder=$sokete_gonder.$str.",".$kontrol.","."2";
